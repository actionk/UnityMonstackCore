...
namespace Plugins.UnityMonstackCore.Utils.Trees
{
    public class KdTree2df<T> : IEnumerable<T> where T : class, IKdTree2dfEntry
    {
        ...
        public IKdTree2dfEntry this[int key]
        {
            get
            {
                if (key >= _count)
        ...
        public KdTree2df<T> FindAll(Predicate<T> match)
        {
            var list = new KdTree2df<T>();
        ...
        protected float _getSplitValue(int level, float2 position)
        {
        ...
        private void _add(KdNode newNode)
        {
            ...
            newNode.right = null;
            newNode.level = 0;
            var parent = _findParent(newNode.entry.Position);
            ...
            var splitNew = _getSplitValue(parent.level, newNode.entry.Position);
        ...
        private KdNode _findParent(float2 position)
        {
        ...
        public T FindClosest(float2 position)
        {
        ...
        /// <returns>closest object</returns>
        public T FindClosest(float2 position, IKdTree2dEntry excluding)
        {
        ...
        public IEnumerable<T> FindClose(float2 position)
        {
        ...
        /// <returns>close object</returns>
        public IEnumerable<T> FindClose(float2 position, float maxDistance)
        {
            var output = new List<T>();
            _findClosest(position, output);
        ...
        protected T _findClosest(float2 position, List<T> traversed = null)
        {
            if (_root == null)
                return null;
        ...
        {
            return _getSplitValue(node.level, node.entry.Position);
        }
