...
{
    public class KdTree3df<T> : IEnumerable<T> where T : class, IKdTree3dfEntry
    {
        ...
        public KdTree3df(bool just2D = false)
        {
            _just2D = just2D;
        }

        public IKdTree3dfEntry this[int key]
        {
            get
            {
                if (key >= _count)
        ...
        public KdTree3df<T> FindAll(Predicate<T> match)
        {
            var list = new KdTree3df<T>(_just2D);
        ...
        /// <returns>closest object</returns>
        public T FindClosest(Vector3 position, IKdTree3dfEntry excluding)
        {
